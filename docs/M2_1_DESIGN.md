...
| 正式对局与 M3 实验 | `deepseek-v4-flash` | 是 |

同一局九名玩家使用同一个模型配置，不依据角色、阶段或任务动态路由。正式档位在 Pro 完整局暴露高延迟和结构化降级后切回 Flash；测试与正式仍是两个独立参数档位，但当前使用同一模型，以避免模型差异干扰 M3 的基线、消融和策略评测。
...
离线部分已经完成：AgentScope DeepSeek 模型适配器会把授权快照转换成一条 system message 和一条 user message，调用框架的 structured output 接口，失败时最多进行一次格式修复。DeepSeek thinking 模式下 AgentScope 只能使用自动工具选择，模型可能结束推理却不提交结构化结果；因此只有狼队规划和重要发言首轮使用 thinking，投票与夜间有限动作等任务首轮直接使用同一模型的 non-thinking 实例。格式修复也固定使用 non-thinking 强制结构化输出。请求异常同样允许进入该修复路径。只有两次均失败时，调用方才使用可审计安全降级；调用记录保存每次尝试的 thinking 状态、失败原因并标记 `fallback_used=true`。
